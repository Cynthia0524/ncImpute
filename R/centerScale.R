...
### Correction in case of user/items with zero ratings
        ind1=which(dtau==0); dtau[ind1]=1e-06
        ind2=which(dgamma==0); dgamma[ind2]=1e-06
        ind1=is.nan(dtau); dtau[ind1]=1
        ind2=is.nan(dgamma); dgamma[ind2]=1    

        crit=sum(dalpha^2)+sum(dbeta^2)+sum(log(dtau)^2)+sum(log(dgamma)^2)  # Converge criterion (99)  
...
### Correction in case of user/items with zero ratings
        ind1=is.nan(beta); beta[ind1]=0
        ind1=is.nan(dbeta); dbeta[ind1]=0
...
### Correction in case of user/items with zero ratings
        ind1=which(alpha==0); alpha[ind1]=1e-06
        ind1=is.na(alpha); alpha[ind1]=1
        ind1=which(tau==0); tau[ind1]=1e-06
        ind1=is.nan(tau); tau[ind1]=1      
...
### Check
### Correction in case of user/items with zero ratings
      ind1=which(dtau==0); dtau[ind1]=1
      ind1=which(dgamma==0); dgamma[ind1]=1
      ind1=is.nan(dalpha); dalpha[ind1]=0
      ind1=is.nan(dbeta); dbeta[ind1]=0 
      ind1=is.nan(dtau); dtau[ind1]=1
      ind2=is.nan(dgamma); dgamma[ind2]=1     
