Ssvd.als <- function (x, J = 2, thresh = 1e-05, lambda = 0, maxit = 100, trace.it = FALSE, warm.start = NULL, final.svd = TRUE){
### Computes soft-thresholded SVD via regularized ALS of the given matrix x of the form sparse or SparsplusLowRank.	
  this.call=match.call()
  n <- dim(x)
  m <- n[2]
  n <- n[1]
  nz=as.numeric(n)*as.numeric(m)
  ...
  while ((ratio > thresh)&(iter<maxit)) {
   ...
   # U=U%*%Bsvd$v
    ## V step
    ## This computes the Frobenius norm of the Difference between x and UB'
